...
local function add_workspace(monitor_id, workspace_id)
	...
	space:subscribe({ "aerospace_workspace_change" }, function(env)
		local selected = env.FOCUSED_WORKSPACE == workspace_id
		space:set({
...
local workspace_ids = { "T", "P", "C", "W", "S", "B", "M", "F" }
for _, workspace_id in ipairs(workspace_ids) do
	add_workspace(1, workspace_id)
end
			...
			if workspace_id == "focused" then
				sbar.exec("aerospace list-workspaces --focused", function(focused_workspace)
					for id in focused_workspace:gmatch("%S+") do
						workspaces[id]:set({ label = icon_line })
					end
				end)
			else
				sbar.animate("tanh", 10, function()
					workspaces[workspace_id]:set({ label = icon_line })
				end)
			end
		end
	)
...
local focused_workspace = ok and ws or ""
